...
- **Widening (Jensen, 2026-08-29):** *anything that is PII, a secret, or a password is redacted or encrypted.* The rule is the category, not a list of field names, and it covers three things the original wording did not:
  ...
  3. **The `auth=` query parameter in both its forms.** The base64 form *is* the account's username and password and must never be constructed by this project (§5.16.1); the `!`-prefixed scoped token is a secret and is redacted like any other.
...
- **Schedule *definition* management (create / edit / delete)** — deliberately out, per AD-7's split: a schedule is a SecuritySpy-owned object Home Assistant interacts with, the same way it does a camera it did not create. Revisit only if real user appetite appears; assignment (FR-12a) covers the arming need without it, and nothing in AD-7 forecloses the larger surface.

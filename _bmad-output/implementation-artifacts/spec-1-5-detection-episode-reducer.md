...
- [x] [Review][Patch] (2026-09-12, independent follow-up review, R1) The 2026-08-30 forward-clamp fix (`_Track.clamp_future`) mutated `start`/`last_any`/`last_qualifying` in place via `min(x, now)` on every authoritative tick. That mutation is one-way and never restored, so an *ordinary, small* clock disagreement between the signal source (SecuritySpy's own clock) and the caller's clock (README recommends `tick(datetime.now(UTC))`, a different clock than the one that stamped the signals) — not just the hour-scale anomaly the fix targeted — could freeze a track's deadline at whatever `now` happened to be on the first tick that noticed the anchor was ahead, closing the episode earlier than the true deadline by an amount that tracked *tick cadence*, not the actual clock disagreement. **Fix:** `clamp_future` replaced by `_Track.observe_now`, which records a separate `skew_ceiling = now + gap` the first time it notices the anchor is ahead, clears it once `now` naturally catches up, and never touches the track's real timestamps. `_Track.deadline` applies the ceiling only when tighter than the true deadline. Consequence: an emitted episode's `last_signal`/`start` are always the true, unmutated values (even when poisoned by a bad future-stamped signal) rather than a silently "corrected" one; worst-case early-closure error is now bounded by the actual clock disagreement, not by when a tick happened to fire. Regression test `test_sustained_small_clock_drift_bounds_worst_case_but_does_not_close_early` added; fails under the old mechanism (3s early close on a 3s drift), passes under the new one. All gates green, 1017 passed.

- [x] [Review][Patch] (2026-09-12, second independent review of the R1 fix above, same day) Two bugs in the `skew_ceiling` mechanism just introduced: (1) the ceiling was set once and frozen for as long as it stayed non-`None`, so a *continuously-active* track under sustained (not anomalous) drift stayed exactly that far ahead of `now` forever, and the frozen ceiling closed it after one gap of real time had passed since the very first tick — regardless of how long the genuine detection continued. (2) `_Track.deadline()` computed `anchor + gap` and returned `None` on `OverflowError` before ever checking `skew_ceiling`, so an anchor near `datetime.max` made the track permanently unclosable by `tick()` even with a valid ceiling set — a regression versus the replaced `clamp_future`, which pulled the anchor down before the addition and so never overflowed. **Fix:** added `_Track.skew_anchor_seen`, the anchor value the current ceiling was last computed against; `observe_now` re-extends the ceiling to `now + gap` whenever the anchor has advanced since (fresh signals keep arriving) and only leaves it frozen when the anchor is unchanged (genuine silence). `deadline()` now falls back to `skew_ceiling` whenever `anchor + gap` overflows, since the ceiling is derived from `now + gap` and cannot itself overflow. Regression tests `test_continuous_activity_under_sustained_drift_does_not_close_early` and `test_an_overflowing_anchor_still_closes_via_the_skew_ceiling` added; both fail under the prior version of the fix. All gates green, 1019 passed.

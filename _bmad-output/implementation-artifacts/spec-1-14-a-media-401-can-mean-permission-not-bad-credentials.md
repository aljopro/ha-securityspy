---
title: "Story 1.14: A media 401 can mean permission, not bad credentials"
type: 'bugfix'
created: '2026-08-29'
...
**Problem:** SecuritySpy 6.21 answers a *permission* failure on the media endpoints with `401`, not `403`. Verified live (§5.9): an account holding **Live**-only permission gets `401` from `++getfile`, `++getfilehb`, `++getfilelb` and `++getpreview` while the same credentials return `200` from `++systemInfo` in the same second. `_map_status` maps every `401` to `SecuritySpyAuthError`, whose documented meaning is "the credentials were rejected outright" -- a claim its docstring attributes to research §4.1/§5.2, which turn out to describe the *settings* pages only. In Home Assistant a `SecuritySpyAuthError` starts a reauth flow, so a Live-only user is asked to re-enter credentials that are already correct, fails identically, and loops forever. The true cause -- this account may not read captures -- is never surfaced.

**Approach:** The response cannot settle it: a permission `401` is byte-identical to a wrong-password `401` (same status, same `WWW-Authenticate`, same 16-byte body). So on a `401` from a media endpoint, and only there, ask a second question the account is known to be allowed: re-read `++systemInfo`. If it succeeds the credentials are valid and the failure is a permission denial, raised as `SecuritySpyPermissionError` carrying the camera the request targeted. If it also returns `401`, the credentials really are bad and the existing `SecuritySpyAuthError` stands.

## Boundaries & Constraints

**Always:** The disambiguation runs **only** on the error path, **only** for the media endpoints, and adds exactly one request to a request that has already failed. `_map_status` stays the single seam that decides what a status means (AD-6); the media paths pass it what they know rather than growing a parallel ladder. A media `SecuritySpyPermissionError` carries the camera number, which the call site always knows -- it is in the path it just built. A successful request is never slowed, never probed, and issues no extra call. If the probe itself fails for any reason other than a clean `200`/`401` answer -- timeout, TLS, connect error -- the original `SecuritySpyAuthError` is raised unchanged: an inconclusive probe must never upgrade or downgrade the verdict. Docstrings on `_map_status`, `SecuritySpyAuthError` and every media accessor are corrected to state that `401` is endpoint-dependent.
...
| Non-media `401` | `++systemInfo` returns `401` | `SecuritySpyAuthError`; no probe issued | Unchanged behaviour |
...
- `aiosecurityspy/src/aiosecurityspy/const.py` -- `ENDPOINT_GET_FILE`, `ENDPOINT_GET_FILE_HIGH_BANDWIDTH`, `ENDPOINT_GET_FILE_LOW_BANDWIDTH`, `ENDPOINT_GET_PREVIEW` (lines 175-187): the four endpoints in scope, and the natural place for the set that defines "media".
...
**Execution:**
- [ ] `aiosecurityspy/src/aiosecurityspy/const.py` -- name the set of media endpoints -- the rule is "these endpoints, not all endpoints", so the boundary belongs where the endpoints are defined.
- [ ] `aiosecurityspy/src/aiosecurityspy/client.py` -- teach the media transport paths to disambiguate a `401` before it escapes, keeping `_map_status` the only place a status is interpreted -- a second status ladder would be exactly the divergence AD-6 exists to prevent.
...
**Acceptance Criteria:**
- Given a Live-only account, when any of the four media endpoints is fetched, then the caller receives a permission error naming the camera, and Home Assistant does not start a reauth flow.

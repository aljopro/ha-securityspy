...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-21-spike-do-securityspy-api-keys-replace-credentials.md`
  ...
  evidence: (resolved; kept for history)
- source_spec: `_bmad-output/implementation-artifacts/spec-1-22-close-story-1-21s-unmet-sub-acs-and-decide-on-shape-based-key-detection.md`
  summary: Add a narrow `API_`-prefix diagnostic warning to the client's exception mapping so an account whose password merely starts with `API_` gets a clearer error than a bare `SecuritySpyAuthError`/401 when the API surface refuses it.
  evidence: Live-confirmed 2026-09-16 (post-done follow-up): a password starting `API_` but not matching the full 36-char key shape succeeds at the SecuritySpy web UI login but is refused (401) on the Basic-auth API surface this library uses, even with no real key on the account. This is silently indistinguishable from a wrong password today (`_map_status`, client.py:1263-1268), and the web UI still working makes the cause hard for an affected operator to spot. See research/securityspy-api-keys-6.22.md's revised recommendation -- adopting this is scoped to a clearer error message only, not an authentication-behavior change, and requires a follow-up correct-course per Story 1.22's boundaries.

...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-22-close-story-1-21s-unmet-sub-acs-and-decide-on-shape-based-key-detection.md`
  summary: RESOLVED 2026-09-16 -- implemented via spec-api-key-prefix-diagnostic-warning.md: `SecuritySpyAuthError` now carries a hedged `API_`-prefix hint (`_ConnectionSettings.password_has_api_key_prefix`, set in `connection.py`; consumed in `client.py`'s 401 branch), never quoting the password, never changing auth behavior.
  evidence: (resolved; kept for history)

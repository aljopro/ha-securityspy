...
**Approach:** Give the library the question as a first-class call: a permission-scoped camera list carrying current state. Add the pure pieces a consumer needs around it — capability predicates that turn the permission bitmask into questions, and a reason for a camera that has left the inventory — so no consumer re-derives any of it. Actions stay on the client and data stays inert.

## Boundaries & Constraints

**Always:** The permission-scoped guarantee is **stated and tested**, not implied: the returned list contains exactly the cameras `++systemInfo` reports, and a camera absent from it never appears by any path. `++camStatus` rows are intersected against that membership and **non-member rows are discarded at the point of receipt**, never returned, never logged (a count may be logged, never a camera number). The cheap-poll path stays cheap: refreshing health must not force a 27 KB `++systemInfo` read when the caller already holds membership. Capability predicates distinguish **permission** from **liveness** -- the mask is not static, and an offline camera loses `PERM_AUDIORCV`/`PERM_AUDIOSND` and regains them on reconnect (research §5.11), so a predicate must never let "the camera is unplugged" read as "you are not allowed". The client remains stateless: it holds no camera inventory between calls.
...
| Disabled camera | Camera disabled in SecuritySpy | Absent from the list -- `++systemInfo` no longer carries it | Not an error |
...
**Execution:**
- [ ] `aiosecurityspy/src/aiosecurityspy/client.py` -- add the permission-scoped camera accessor with current state, and a health-refresh path that does not re-read `++systemInfo` -- the coordinator polls on a cycle and a 27 KB read per cycle is not viable.
